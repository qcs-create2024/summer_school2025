...
dt=2.0; %dt=bigt*1e-3;
t=0;
NL=1; ROT=1; f=ROT*f0; NH=1; EDDY_VISC=1;

nhfact=1./(1+NH*k2*H*H/6);
eddyviscfact2=1./(1+EDDY_VISC*k2*1e-1);
eddyviscfact3=1./(1+EDDY_VISC*k2*1e0);
eddyviscfact4=1./(1+EDDY_VISC*k2*1e+1);
...
if drawpics==1
 ...
 t=t+dt;
    ...
    uf3=fft(um3);zf3=fft(zm3);uhef3=fft(um3.*zm3);uuf3=0.5*fft(um3.^2); vf3=fft(vm3);
    
    un3=um3+dt*(real(ifft((-i*NL*k.*uuf3-i*g*k.*zf3+f*vf3))));
    vn3=vm3+dt*(um3.*real(ifft(-i*NL*k.*vf3))-f*um3);
    zn3=zm3+dt*(real(ifft(-i*k.*uhef3)));
    
    uf4=fft(um4);zf4=fft(zm4);uhef4=fft(um4.*zm4);uuf4=0.5*fft(um4.^2); vf4=fft(vm4);
    
    un4=um4+dt*(real(ifft((-i*NL*k.*uuf4-i*g*k.*zf4+f*vf4))));
    vn4=vm4+dt*(um4.*real(ifft(-i*NL*k.*vf4))-f*um4);
...
for ii=1:numouts
 for jj=1:numsteps
    ...
    un2=real(ifft(eddyviscfact2.*myfilter.*fft(up2)));
    vn2=real(ifft(eddyviscfact2.*myfilter.*fft(vp2)));
    zn2=real(ifft(myfilter.*fft(zp2)));

    uf3=fft(un3);zf3=fft(zn3);uhef3=fft(un3.*(H+NL*(zn3-H)));uuf3=0.5*fft(un3.^2); vf3=fft(vn3);
   
    up3=um3+2*dt*(real(ifft(nhfact.*(-i*NL*k.*uuf3-i*g*k.*zf3+fnow+f*vf3))));
    vp3=vm3+2*dt*(un3.*real(ifft(-i*NL*k.*vf3))-f*un3);
    zp3=zm3+2*dt*(real(ifft(-i*k.*uhef3)));
    
    um3=un3;vm3=vn3;zm3=zn3;
    un3=real(ifft(eddyviscfact3.*myfilter.*fft(up3)));
    vn3=real(ifft(eddyviscfact3.*myfilter.*fft(vp3)));
    zn3=real(ifft(myfilter.*fft(zp3)));

    uf4=fft(un4);zf4=fft(zn4);uhef4=fft(un4.*(H+NL*(zn4-H)));uuf4=0.5*fft(un4.^2); vf4=fft(vn4);
   
    up4=um4+2*dt*(real(ifft(nhfact.*(-i*NL*k.*uuf4-i*g*k.*zf4+fnow+f*vf4))));
    vp4=vm4+2*dt*(un4.*real(ifft(-i*NL*k.*vf4))-f*un4);
    zp4=zm4+2*dt*(real(ifft(-i*k.*uhef4)));
    
    um4=un4;vm4=vn4;zm4=zn4;
    un4=real(ifft(eddyviscfact4.*myfilter.*fft(up4)));
    vn4=real(ifft(eddyviscfact4.*myfilter.*fft(vp4)));
...
if drawpics==1
      ...
      ylabel('scaled eta')
     % legend('bottom drag and f','no drag','no NL','Location','SouthWest')
      subplot(3,1,2)
      ...
      legend('drag','low eddy visc','med eddy visc','high eddy visc')
      axis([0 1 -0.075 0.075])
      grid on
      ylabel('scaled v')
    if compute_spec==1 
