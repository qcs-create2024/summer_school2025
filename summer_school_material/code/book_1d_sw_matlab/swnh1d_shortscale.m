%% This code solves the 1D Shallow Water equations with nonhydrostatic corrections
%% in a periodic deomain in physical formulation
% not in conservative form
% surface is z1, total depth H
% formulation includes bottom and surface stress
...
save_files = 0;
%% physical parameters
g = 9.81*0.01; %the fraction is for reduced gravity models of internal waves
f0 = 0e-4;
c_d = 0.00025e0;
cw = 0.0001;
rhoa = 1.2;
...
%Build the filter for treating aliasing
filtorder = 8;
kmax = max(k(:));
cutoff = 0.65;
kcrit = kmax*cutoff;
...
um1 = zeros(size(xf));
vm1 = zeros(size(xf));
zm1 = H*(1-0.2*0.5*(tanh((xf-1e2)/5e0)-tanh((xf+1e2)/5e0)));
...
uhef1 = fft(um1.*zm1); uuf1 = 0.5*fft(um1.^2); vf1 = fft(vm1);
% for bottom drag
...
uf2 = fft(um2); zf2 = fft(zm2);
uhef2 = fft(um2.*zm2); uuf2 = 0.5*fft(um2.^2); vf2 = fft(vm2);

un2 = um2+dt*(real(ifft((-i*NL*k.*uuf2-i*g*k.*zf2+f*vf2))));
vn2 = vm2+dt*(um1.*real(ifft(-i*NL*k.*vf2))-f*um2);
zn2 = zm2+dt*(real(ifft(-i*k.*uhef2)));

uf3 = fft(um3); zf3=fft(zm3);
uHf = fft(um3.*H); uuf3 = 0.5*fft(um3.^2); vf3 = fft(vm3);
...
uf4 = fft(um4); zf4 = fft(zm4); 
uhef4 = fft(um4.*zm4); uuf4 = 0.5*fft(um4.^2); vf4 = fft(vm4);
...
%% time stepping loop for four different cases
for ii = 1:numouts
 for jj = 1:numsteps
     % this could be used for forcing
    fnow = 0*fft(1e-2*sin(2*pi*t/(5*16.4873))*cos(pi*xf/L));
    t = t+dt;
    uf1 = fft(un1);zf1 = fft(zn1);
    uhef1 = fft(un1.*(H+NL*(zn1-H))); uuf1 = 0.5*fft(un1.^2);
    vf1 = fft(vn1);
    % for bottom drag
    ...
    % rotate the "now" variables to the "minus" variables
    um1 = un1;vm1 = vn1;zm1 = zn1;
    % roatte the "plus" variables to the "now" variables
    un1 = real(ifft(myfilter.*fft(up1)));
    vn1 = real(ifft(myfilter.*fft(vp1)));
    zn1 = real(ifft(myfilter.*fft(zp1)));
 
    uf2 = fft(un2); zf2 = fft(zn2);
    uhef2 = fft(un2.*(H+NL*(zn2-H))); uuf2 = 0.5*fft(un2.^2);
    vf2 = fft(vn2);
    ...
    uf3 = fft(un3); zf3 = fft(zn3);
    uH3 = fft(un3.*H); vf3 = fft(vn3);
    ...
    uf4 = fft(un4); zf4 = fft(zn4);
    uhef4 = fft(un4.*(H+NL*(zn4-H))); uuf4 = 0.5*fft(un4.^2);
    vf4 = fft(vn4);
    spd = sqrt(un4.*un4+vn4.*vn4);
    drgxf = fft(-c_d.*un4.*spd);
    drgyf = fft(-c_d.*vn4.*spd);
    
    up4 = um4+2*dt*(real(ifft((-i*NL*k.*uuf4-i*g*k.*zf4+fnow+drgxf))));
...
if drawpics==1
      ...
      axis([0 1 -0.025 0.25])
      grid on
      legend('full','no drag','linear','classical SW')
      title(['t = ' num2str(t/bigt,2)])
%      title(['t = ' num2str(t/bigt,2) ' full (black), no drag (blue dashed), linearized (green), classical SW (red)'])
      ylabel('scaled eta')
